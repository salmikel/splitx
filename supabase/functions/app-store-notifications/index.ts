...
//
// Verifies App Store Server Notifications V2 from Apple (JWS: x5c certificate
// chain anchored to Apple Root CA - G3, then the ES256 signature) and writes
// profiles.premium_until with the service role. This is the tamper-proof source
// of truth for the Premium entitlement — a client cannot forge it, and
// migration 013 locks premium_until to the service role.
//
// Deploy with verify_jwt = false (Apple sends no Supabase JWT). Fails closed:
// any verification error rejects the request and never grants entitlement.
// Users are mapped via appAccountToken (the iOS app sets it to the user's UUID
// at purchase time).
...
Deno.serve(async (req) => {
  ...
  const userId = (tx.appAccountToken as string | undefined)?.trim()
  if (!userId) return json({ ok: true, note: 'no appAccountToken' })

  // Entitlement window: cleared on revocation/refund, else the expiry date.
  let premiumUntil: string | null = null
  if (!tx.revocationDate && typeof tx.expiresDate === 'number') {
    const d = new Date(tx.expiresDate)
    premiumUntil = isNaN(d.getTime()) ? null : d.toISOString()
  }
  ...
  console.log('updated', userId, '-> premium_until', premiumUntil, '(', notification.notificationType, ')')
  return json({ ok: true })
})
